@extends('layouts.app')
@section('title', 'Edit Tariff Label Values')
@section('content')
                    ...
                    <div class="box-body">
                        <table class="table table-bordered table-striped nowrap">
                            <thead>
                               ...
                               <?php /*?> <th>Action <i data-toggle="tooltip" data-placement="right" title="Click to action" class="fa fa-info-circle" aria-hidden="true"></i></th><?php */ ?>
                            </thead>
                            ...
                            <tbody id="ivr_level_body">
                                ...
                                $i=0;
                                @endphp
                                @foreach($tariff_label_value as $values)
                                ...
                                <tr>
                                        ...
                                        <div class="input-group col-md-12">
                                            <select class="form-control" name='tariff_id'>
                                                @foreach($tariff_plans as $plan)
                                                    <option @if($plan->id == $values['tariff_id']) selected @endif value="{{$plan->id}}">{{$plan->title}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </td>
                                        ...
                                        <div class="input-group col-md-12">
                                            <select class="form-control" name='phone_countries_id[]'>
                                                @foreach($phone_country as $phone)
                                                    <option @if($phone->id == $values['phone_countries_id']) selected @endif  value="{{$phone->id}}">{{$phone->country_name}} (+{{$phone->country_code}})</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </td>
                                  ...
                                  <td>
                                        <div class="input-group col-md-12">
                                            <input   type="hidden" class="form-control" name="tariff_value_id[]" value="{{$values['id']}}" id="" required="">
                                            <input onkeypress="return isNumberKey(event)"  type="text" class="form-control" name="rate[]" value="{{$values['rate']}}" id="campaign_name" required="">
                                        </div>
                                    </td>
                            ...
                            <tfoot>
                                <tr>
                                    <td>
                                        <button class="btn btn-primary" type="submit"><i class="fa fa-refresh fa-lg"></i> 
                                                        Update
                                        </button>
